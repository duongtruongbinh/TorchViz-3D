import {
  llmUnlearningCitationEvidence,
  llmUnlearningCitationLinkOnlyExceptions,
} from '../src/content/learning/llm-unlearning/citationEvidence.ts';
...
const results = await Promise.allSettled(llmUnlearningCitationEvidence.map(auditEvidence));
let failures = 0;
for (const [index, result] of results.entries()) {
  if (result.status === 'fulfilled') {
    if (result.value.status === 'manual-required') {
      console.log(`◇ ${llmUnlearningCitationEvidence[index].id}: manual review required — ${result.value.message}`);
    } else {
      console.log(`✓ ${llmUnlearningCitationEvidence[index].id}: verified`);
    }
    continue;
  }
  failures += 1;
  const evidence = llmUnlearningCitationEvidence[index];
...
if (failures > 0) {
  ...
  const exceptionResults = await Promise.all(llmUnlearningCitationLinkOnlyExceptions.map(auditLinkOnlyException));
  for (const [index, message] of exceptionResults.entries()) {
    console.log(`◇ ${llmUnlearningCitationLinkOnlyExceptions[index].id}: ${message}`);
  }
