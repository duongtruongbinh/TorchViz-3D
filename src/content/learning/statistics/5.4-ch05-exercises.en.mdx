...
2. We will now derive the probability that a given observation is part of a bootstrap sample. Suppose that we obtain a bootstrap sample from a set of _n_ observations.
   ...
   - (c) Argue that the probability that the _j_ th observation is _not_ in the bootstrap sample is (1 _−_ 1 _/n_ )^(_n_) .
...
- (h) Use the bootstrap to estimate the standard error of _µ_ ˆ0 _._ 1. Com- `percentile()` ment on your findings.
