...
1. It was mentioned in this chapter that a cubic regression spline with one knot at _ξ_ can be obtained using a basis of the form _x_ , _x_^(2) , _x_^(3) , ( _x − ξ_ )^(3) +^(,where()^(_x −ξ_)3) +^(= ()^(_x −ξ_)3if)^(_x &gt; ξ_andequals0otherwise.) We will now show that a function of the form
...
(d) Show that _f_ 1^(_′_()^(_ξ_) =)^(_f ′_) 2^(()^(_ξ_).Thatis,)^(_f ′_()^(_x_)iscontinuousat)^(_ξ_.) (e) Show that _f_ 1^(_′′_()^(_ξ_) =)^(_f ′′_) 2^(()^(_ξ_).Thatis,)^(_f ′′_()^(_x_)iscontinuousat)^(_ξ_.)
