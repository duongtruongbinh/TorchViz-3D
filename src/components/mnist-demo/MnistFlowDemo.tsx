...
export const DemoControls: React.FC<{
  ...
  const activeStop = segment.activeStop;
  const currentLabel = activeStop?.label ?? t.input;
  ...
  return (
    <div className="absolute left-4 top-4 z-30 w-[min(14rem,calc(100%-5.5rem))] rounded-lg border border-white/15 bg-zinc-950/70 shadow-2xl backdrop-blur-md pointer-events-auto">
      <div className="flex items-center gap-3 px-3 py-2.5 border-b border-white/10">
        <div className="w-10 h-10 rounded-md overflow-hidden border border-sky-300/40 bg-black shrink-0">
          {dataUrl && <img src={dataUrl} alt="" className="w-full h-full object-cover [image-rendering:pixelated]" />}
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-3">
            <p className="text-[11px] font-bold uppercase text-sky-200 tracking-wider">{t.title}</p>
            <span className="text-[10px] font-mono text-zinc-400 tabular-nums">{progressPct}%</span>
          </div>
          <p className="text-xs text-zinc-200 truncate">{currentLabel}</p>
        </div>
      </div>
      ...
      <div className="px-3 py-2.5 flex flex-col gap-2.5">
        <div className="flex items-center gap-2">
          <button
            type="button"
            className="w-8 h-8 flex items-center justify-center rounded-md border border-zinc-600/70 bg-zinc-900/70 text-zinc-300 hover:text-white hover:bg-zinc-800 transition-colors"
            onClick={() => stepBy(-1)}
            title={t.previous}
          ...
          <button
            type="button"
            className="w-9 h-8 flex items-center justify-center rounded-md border border-sky-400/60 bg-sky-500/20 text-sky-100 hover:bg-sky-500/30 transition-colors"
            onClick={() => {
              if (playing) {
                onPlayingChange(false);
                return;
        ...
        <select
          className="h-8 w-full rounded-md border border-zinc-600/70 bg-zinc-950/80 px-2 text-xs text-zinc-200 outline-none focus:border-sky-400"
          value={selectedIndex}
          aria-label={t.jumpTo}
          onChange={(event) => {
            onPlayingChange(false);
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between gap-2">
            <label className="text-[11px] font-bold uppercase tracking-wider text-zinc-400" htmlFor="mnist-demo-speed">
              {t.speed}
            </label>
            <span className="text-right text-[11px] font-mono text-sky-100 tabular-nums">
