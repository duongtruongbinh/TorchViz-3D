...
export default function Header({
  ...
  const text = getStrings(language).reinforcementLearning;
    ...
    <header className="border-b border-zinc-800 bg-zinc-950/92 px-5 py-4 shadow-xl shadow-black/25">
      <div className="mx-auto flex max-w-[1480px] flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 text-xs font-bold uppercase text-emerald-200">
            <button
              type="button"
          ...
          <h1 className="mt-2 text-xl font-black text-white">{text.title}</h1>
          <p className="mt-1 max-w-2xl text-sm text-zinc-500">{text.description}</p>
        </div>
          ...
          <div className="flex h-8 overflow-hidden rounded-md border border-[var(--border)] bg-[var(--surface-elevated)]">
            {(['path', 'review'] as const).map((item) => (
              <button
                ...
                className={`px-3 text-xs font-bold transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-300/35 ${
                  mode === item ? 'bg-emerald-400/18 text-emerald-100' : 'text-[var(--text-muted)] hover:bg-[#3f3f46] hover:text-white'
                }`}
