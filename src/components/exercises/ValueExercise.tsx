...
export const ValueExercise: React.FC<{
  ...
  language: 'en' | 'vi';
  onClose: () => void;
}> = ({ isOpen, exerciseId, node, fallbackModal = false, t, language, onClose }) => {
  const model = useMemo(() => (
  ...
  return (
    <LearningDrawer
      isOpen={isOpen}
      title={model?.title ?? t.exercises}
      subtitle={model?.subtitle ?? node?.op_type}
    ...
    >
      {!model ? (
        ...
        <div className="space-y-5 overflow-y-auto p-5">
          <section className="rounded-lg border border-zinc-800 bg-gradient-to-br from-zinc-900/50 to-zinc-950/50 p-4 shadow-md">
            <div className="pl-3.5 border-l-2 border-sky-500 text-sm font-medium text-zinc-100/95 leading-relaxed">
              {model.prompt}
            </div>
            <div className="mt-3 flex flex-wrap gap-1.5">
              {model.configRows.map((row) => (
                <span key={row} className="rounded-md border border-sky-500/10 bg-sky-950/20 px-2.5 py-1 font-mono text-[10px] font-semibold text-sky-200 shadow-inner">
                  {row}
                </span>
              ))}
            </div>
          ...
          <section className="grid gap-3 sm:grid-cols-2">
            {model.displaySections.map((section) => (
              <div key={section.title} className="rounded-lg border border-zinc-850 bg-zinc-950/40 p-3 shadow-inner">
                <div className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 mb-2 border-b border-zinc-900/60 pb-1.5">
                  {section.title}
                </div>
                <div className="mt-2 space-y-1 font-mono text-xs text-sky-200/90 leading-relaxed">
