...
test('typed catalog materializes domain metadata and content lifecycle counts', () => {
  ...
  assert.deepEqual(
    ...
    { available: 30, next: 2, locked: 595 },
  );
  assert.equal(learningCatalog.lessons.filter((lesson) => lesson.contentStatus === 'published').length, 26);
...
test('only LLM and tagged CV exercise lessons carry authored content', () => {
  ...
  assert.equal(publishedLessons.filter((lesson) => lesson.domainId === 'llm-ai-engineering').length, 22);
