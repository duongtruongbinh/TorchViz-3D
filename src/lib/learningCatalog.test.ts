...
test('typed catalog materializes domain metadata and content lifecycle counts', () => {
  ...
  assert.deepEqual(
    ...
    { available: 165, next: 2, locked: 552 },
  );
  assert.equal(learningCatalog.lessons.filter((lesson) => lesson.contentStatus === 'published').length, 167);
...
test('LLM, Statistics, and tagged CV exercise lessons carry authored content', () => {
  ...
  assert.equal(publishedLessons.filter((lesson) => lesson.domainId === 'statistics').length, 101);
