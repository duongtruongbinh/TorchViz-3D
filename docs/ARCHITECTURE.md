...
validated by pure helpers in `src/core/learning/*`. The catalog has 696 navigable
nodes, including four canonical CV exercise lessons. Two hundred
Vietnamese-first LLM lessons and four Vietnamese-first CV exercise lessons have
authored MDX; the other 492 nodes use one shared localized placeholder. TOCs own
