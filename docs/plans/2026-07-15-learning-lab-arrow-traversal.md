...
- **Footer layout.** `LessonDetail` keeps the existing section pager (`← Back` / step count / `Next →`, or green `Too easy!` on the last section). A new, visually separated **lesson-arrow row** is added above it with `↑ Previous lesson` / `Next lesson ↓` (up/down icons). The two rows are separated by a divider so the section layer and lesson layer stay distinct.
